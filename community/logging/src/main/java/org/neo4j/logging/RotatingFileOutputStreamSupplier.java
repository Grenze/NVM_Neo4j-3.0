...
import org.neo4j.logging.org.neo4j.logging.fs.DefaultFileSystemAbstraction;
...
public class RotatingFileOutputStreamSupplier implements Supplier<OutputStream>, Closeable
{
    ...
    private final DefaultFileSystemAbstraction logFileSystem = new DefaultFileSystemAbstraction();
    ...
    public OutputStream get()
    {
        if ( !closed.get() && !rotating.get() )
        {
            ...
            if ( rotationDelayExceeded() && rotationThresholdExceeded() ||
                    ...
                    !logFileSystem.fileExists( outputFile ) )
            {
                rotate();
            }
        }
    ...
    private boolean rotationThresholdExceeded()
    {
        ...
        return logFileSystem.fileExists( outputFile ) && rotationThresholdBytes > 0 &&
                logFileSystem.getFileSize( outputFile ) >= rotationThresholdBytes;
            ...
            try
            {
                try
                {
                    ...
                    try
                    {
                        ...
                        if ( logFileSystem.fileExists( outputFile ) )
                        {
                            shiftArchivedOutputFiles();
                            //fileSystem.renameFile( outputFile, archivedOutputFile( 1 ) );
                            logFileSystem.renameFile( outputFile, archivedOutputFile( 1 ) );
                        }
                    }
                    catch ( Exception e )
                    {
    ...
    private OutputStream openOutputFile() throws IOException
    {
        ...
        return createOrOpenAsOuputStream(logFileSystem, outputFile, true );
    }
    ...
    private void shiftArchivedOutputFiles() throws IOException
    {
        for ( int i = lastArchivedOutputFileNumber(); i > 0; --i )
        {
            ...
            if ( i >= maxArchives )
            {
                ...
                logFileSystem.deleteFile( archive );
            }
            else
            {
                //fileSystem.renameFile( archive, archivedOutputFile( i + 1 ) );
                logFileSystem.renameFile( archive, archivedOutputFile( i + 1 ) );
            }
        }
    }
    ...
    private int lastArchivedOutputFileNumber()
    {
        ...
        while ( logFileSystem.fileExists( archivedOutputFile( i ) ) )
        {
            i++;
        }
        return i - 1;
